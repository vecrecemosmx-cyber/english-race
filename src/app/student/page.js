// Pieza 5 de 5: ORQUESTADOR MAESTRO DE LA PLATAFORMA DE INMERSIÓN
// Guarda en: src/app/student/page.js (Longitud segura < 4,000 caracteres)
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 font-sans flex overflow-hidden">
      <aside className={`bg-slate-950 border-r border-slate-800 flex flex-col justify-between transition-all duration-300 ${sidebarVisible ? 'w-64 p-5' : 'w-0 p-0 overflow-hidden border-r-0'}`}>
        <div className="flex flex-col gap-6">
          ...
          <nav className="flex flex-col gap-2">
            ...
            <button onClick={() => { setPanelModo('practicar'); setLeccionActiva(null); }} className="w-full text-left py-3 px-4 rounded-xl font-bold text-sm bg-sky-600 text-white font-black shadow-lg">🎯 Practicar (YouGlish AI)</button>
          </nav>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
        <header className="bg-slate-950 border-b border-slate-800 h-16 flex items-center justify-between px-6">
          ...
          <input type="text" placeholder="🔍 Modo Buscador Libre..." className="w-full max-w-sm bg-slate-900 border border-slate-800 rounded-full py-2 px-4 text-xs font-bold text-slate-300" />
        </header>
          ...
          {panelModo === 'split' && leccionActiva && (
            <div className="w-1/2 bg-slate-950 border-r border-slate-800 p-6 overflow-y-auto flex flex-col gap-4">
              <div className="flex justify-between items-center border-b border-slate-800 pb-3">
                ...
                <button onClick={() => setPanelModo('practicar')} className="text-xs font-black uppercase text-slate-500">[✕ Expandir Práctica]</button>
              </div>
          ...
          <div className={`${panelModo === 'split' ? 'w-1/2' : 'w-full'} flex flex-col overflow-y-auto p-6 gap-6 transition-all duration-300`}>
            {!fraseActual && (
              <form onSubmit={dispararProcesamientoSueño} className="bg-slate-950 border border-slate-800 p-6 rounded-3xl flex flex-col gap-4 shadow-xl">
                ...
                <textarea value={sueñoTexto} onChange={(e) => setSueñoTexto(e.target.value)} placeholder="Ej. Quiero viajar a Nueva York y trabajar en una empresa de tecnología..." className="w-full h-24 bg-slate-900 border border-slate-800 rounded-2xl p-4 text-sm font-semibold text-slate-200 resize-none" />
