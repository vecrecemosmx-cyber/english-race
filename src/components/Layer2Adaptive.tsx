...
export const Layer2Adaptive: React.FC<Layer2AdaptiveProps> = ({
  ...
  // Respaldo de video si existe en layer2
  ...
  return (
    <div className="mt-6 rounded-3xl border-2 transition-all p-5 md:p-8 shadow-xl bg-gradient-to-b from-blue-50/70 to-white dark:from-slate-900/90 dark:to-slate-950 border-blue-200 dark:border-blue-900/50">
      
      {/* Cabecera */}
      <div className="flex items-center justify-between border-b border-blue-100 dark:border-slate-800 pb-4 mb-5">
        <div className="flex flex-wrap items-center gap-3">
          <span className="px-3 py-1 text-xs font-bold uppercase tracking-wider bg-blue-100 dark:bg-blue-950/60 text-blue-700 dark:text-sky-300 rounded-full border border-blue-200 dark:border-blue-800/50">
            Cognitive Scaffolding (Capa 2)
          </span>
          <span className="text-sm text-slate-600 dark:text-slate-400 font-medium">
            Palabra clave: <strong className="text-slate-900 dark:text-white">"{layer2.keyword}"</strong> ({layer2.partOfSpeech})
          </span>
        </div>
        <button
          onClick={onClose}
          className="text-xs text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white font-bold px-3 py-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition"
        >
          ✕ Cerrar
        </button>
      </div>
      ...
      <div className="mb-6 rounded-2xl bg-slate-950 text-slate-200 p-4 font-mono text-xs border border-slate-800 shadow-md">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-[11px]">
          ...
          <div>
            ...
            <strong className="text-sky-400">"{targetComplement || '(Sin complemento)'}"</strong>
          </div>
          <div>
            ...
            <strong className={isSearchingVideo ? 'text-amber-400 animate-pulse' : 'text-emerald-400'}>
              {isSearchingVideo ? 'Buscando en YouTube / Supabase...' : (activeVideo?.matchType ?? 'Finalizada')}
            </strong>
          </div>
        </div>
      ...
      <div className="mb-5 rounded-2xl p-5 border shadow-sm bg-white dark:bg-slate-900 border-blue-100 dark:border-slate-800">
        <h4 className="text-xs font-bold uppercase tracking-wider text-blue-600 dark:text-sky-400 mb-1.5">
      ...
      <div className="mb-5 rounded-2xl p-5 border shadow-sm bg-white dark:bg-slate-900 border-amber-100 dark:border-slate-800">
        <div className="flex items-center gap-2 mb-2">
          <span className="text-xs font-bold uppercase tracking-wider text-amber-700 dark:text-amber-400">
            {layer2.secondaryTechnique.title}:
          </span>
          <span className="text-xs px-2.5 py-0.5 rounded font-mono font-bold bg-amber-100 dark:bg-amber-950/60 text-amber-900 dark:text-amber-300 border border-amber-200 dark:border-amber-800/50">
            Antónimo: {layer2.secondaryTechnique.oppositeWord}
          </span>
        </div>
        <ul className="space-y-1.5">
          {layer2.secondaryTechnique.contrastPhrases.map((phrase, idx) => (
            <li key={idx} className="text-sm md:text-base text-slate-800 dark:text-slate-200 flex items-start gap-2">
      ...
      {isSearchingVideo && !activeVideo ? (
        <div className="mt-5 p-8 rounded-2xl bg-slate-900 text-white text-center animate-pulse border border-slate-800">
          <div className="inline-block h-8 w-8 border-2 border-blue-400 border-t-transparent rounded-full animate-spin mb-3"></div>
          <p className="text-sm font-bold uppercase tracking-widest text-sky-400">
            Escaneando transcripciones en vivo...
          </p>
        ...
        <div className="w-full">
          ...
          {activeVideo.matchType === 'exact_full' && (
            <div className="mb-3 p-3 rounded-xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 text-emerald-800 dark:text-emerald-300 text-xs font-bold flex items-center gap-2">
              <span>✔</span>
          ...
          {activeVideo.matchType === 'collocation_match' && (
            <div className="mb-3 p-3.5 rounded-xl bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800 text-blue-900 dark:text-sky-200 flex items-start gap-2.5 shadow-sm">
              <span className="text-base leading-none mt-0.5">ℹ️</span>
              <div>
                <p className="text-xs font-bold">
                  No se encontró la frase exacta, pero se ha encontrado una frase similar:
                </p>
                <p className="text-sm md:text-base font-extrabold text-blue-700 dark:text-sky-300 mt-0.5 font-mono">
          ...
          {activeVideo.matchType === 'core_structure_only' && (
            <div className="mb-3 p-3.5 rounded-xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 text-amber-900 dark:text-amber-200 flex items-start gap-2.5 shadow-sm">
              <span className="text-base leading-none mt-0.5">🎯</span>
              <div>
                <p className="text-xs font-bold">
                  Estructura base encontrada en audio real:
                </p>
                <p className="text-sm md:text-base font-extrabold text-amber-700 dark:text-amber-300 mt-0.5 font-mono">
                  "{activeVideo.matchedPhrase}"
                </p>
              </div>
            </div>
        ...
        <div className="mt-5 p-8 rounded-2xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 text-amber-900 dark:text-amber-200 text-center">
